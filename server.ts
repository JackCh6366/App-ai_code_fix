...
// Set request payloads limits high enough for larger code files
...
function getGenAI(): GoogleGenAI {
  if (!aiClient) {
    ...
    if (!key) {
      throw new Error("找不到 GEMINI_API_KEY 環境變數。請在 AI Studio 中設定 Secrets！");
    }
...
- language: 預測、識別此程式碼的最適合副檔名/語言形態（例如：'html', 'css', 'javascript', 'typescript', 'tsx', 'json', 'markdown'）。`;
    ...
    // Include structural code context as a helper prefix
    const contextPrompt = `
...
`;
    ...
    // If there is history, we append previous chats. But to feed the prompt and current code structure cleanly,
    // we make sure the current turn represents the dynamic prompt together with contextual code.
...
// Configure Vite dynamic middleware for local dev OR serve static build for deployment
